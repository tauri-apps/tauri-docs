...
[Snap packages](https://snapcraft.io/about) are an alternative distribution format for Linux that do not rely on the system installed packages. In order to build snap packages using the CLI, the [`snapcraft`](https://github.com/snapcore/snapcraft) package must be installed on the developer's machine.

After building a snap package, it can be signed and uploaded to the [Snap Store](https://snapcraft.io) for easier distribution. More information about Snaps, how to use them, and usage for end users can be found on the [Snapcraft about page](https://snapcraft.io/about)
