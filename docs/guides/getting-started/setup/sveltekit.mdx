...
![Application Preview](./assets/sveltekit/sveltekit.png)

## Create the Frontend

[SvelteKit] is a Svelte Frontend which is primarily designed for Server-Side Rendering (SSG). To make SvelteKit work with Tauri we are going to disable SSR and use [`@sveltejs/adapter-static`] to create a frontend based on Static-Site Generation (SSG).

SvelteKit comes with a scaffolding utility similar to [`create-tauri-app`] that can quickly setup a new project with a number of customization options. For this guide we will select the [TypeScript] template, with ESLint and Prettier enabled.
...
2. _App template_  
   We will select `Skeleton project` for a barebones template. If you want to play around with a more complete SvelteKit example you can select `SvelteKit demo app`.
...
5. _Browser testing_  
   SvelteKit offers built-in Playwright support for browser testing. Since Tauri apis don't work in Playwright, we assume you disable this option. Check out our [WebDriver documentation] for alternatives using Selenium or WebdriverIO instead of Playwright.
...
```shell
yarn install -D @sveltejs/adapter-static@next
```
...
import adapter from '@sveltejs/adapter-static' // This was changed to adapter-static!
...
Note that if you prefer a Single-Page Application (SPA) mode over SSG, you need to change the adapter configurations and `+layout.ts` according to the [adapter docs].
...
![Application Window](./assets/sveltekit/sveltekit-dev.png)
...
With the library installed, you can modify your `+page.svelte` file to call the Command:
