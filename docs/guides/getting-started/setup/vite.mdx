...
You'll use `../dist` for step 3 (this is Vite's default output directory) and `http://localhost:3000` (or whichever port you've set in your Vite config) for step 4.
...
```json title=src-tauri/tauri.conf.json
{
  "build": {
    ...
    // this command with execute when you run `tauri dev`
