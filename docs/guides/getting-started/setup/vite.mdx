...
When starting the frontend via the `vite` command, Vite will look for a config file named `vite.config.ts` inside the project root. We want to customize this file to get the best compatibility with Tauri. Since it is not created by the scaffolding above, create the `vite.config.ts` file in the root directory of the project and add the following content: 
