...
While there are various options listed to create the frontend framework, only one method is required. They are meant to provide various options based on preference, rather than individual steps to be completed sequentially in a single workflow. Or you can just run 'create tauri-app' for a more comprehensive but less initially-customizable development. 
