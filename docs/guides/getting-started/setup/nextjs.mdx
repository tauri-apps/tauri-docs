...
The `benefit` of it is React pre rendered in static HTML/JavaScript version, that means your page can load faster.
React doesn't have to render the html on client side on the first load it will just hydrate it if needed.
But the `downside` is we cannot use `getServerSideProps` or use any data fetching for render our page for a request.

:::tip

You can use `getStaticProps` to generate HTML/JavaScript pages at build time.
Since it's executed in nodejs context calling tauri api wont work.
