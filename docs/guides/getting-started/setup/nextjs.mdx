...
Using the `dev` script will open the development website at any available server. So, to make it permanent, modify the `dev` script in `package.json` in the root of the project:
...
Now, your app will run on https://localhost:2005 everytime you run the `dev` or `tauri dev` script.

When using the `build` command, it does not generate a static site automatically. Next.js provides us a way to generate a static HTML/Javascript website. Update the `build` script in the `package.json` in the root of your project:
...
The downside of creating a static site will be that you won't be able to use any type of data fetching like `getServerSideProps`.

:::info

You can still use `getStaticProps` to generate HTML/JavaScript pages at build time. Since it is executed in nodejs context, calling the tauri api wont work. You can learn more about NextJS SSG on [official documentation](https://nextjs.org/docs/basic-features/pages#static-generation-recommended)
...
But before you do, let's quickly configure Tauri so it **automatically** start's the nextjs development server when you type `tauri dev` and automatically runs the build script when you run `tauri build`:
