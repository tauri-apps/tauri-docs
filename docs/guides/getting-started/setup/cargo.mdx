---
description: Build a Tauri app without Node using Cargo
...
This guide will walk you through creating your first Tauri app using just Cargo. This is if you don't want to use a Node package manager.
...
<TauriInit
  ...
  useCargo={true}
/>
And that's it! Now you can run the following command in your terminal to start a
...
We would normally be recommending the [`@tauri-apps/api`] package here, but since we're using cargo for this guide, please enable [`withGlobalTauri`] in your `tauri.conf.json` file:
