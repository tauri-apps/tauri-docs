...
## Exclude src-tauri from `tsconfig.json`

If you are using Typescript you might wanna exclude the `src-tauri` directory. It might prevent from type compilation error on build time.
```json
"exclude": [
    "node_modules",
    "src-tauri"
...
```
In `tscongig.json`, append `"src-tauri"` to `"exclude" : [...]`. It should already be containing "node_modules".
