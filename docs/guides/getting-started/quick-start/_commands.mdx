...
<span>{props.showGlobal &&

<span>
We would normally be recommending the <code>@tauri-apps/api</code> package here, but since we're not using a bundler for this guide, please enable <a href="../../../api/config#buildconfig.withglobaltauri"><code>withGlobalTauri</code></a> in your <code>tauri.conf.json</code> file:
