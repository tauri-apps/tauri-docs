...
generateCorePermissionsPage(
  '../tauri/crates/tauri/permissions',
  '../../src/content/docs/reference/acl/core-permissions.md'
);

function generateCorePermissionsPage(permissionsDir: string, outputFile: string) {
  if (!existsSync(permissionsDir)) {
    throw Error(
      `Could not find the Tauri core permissions directory ${permissionsDir}. Is the Tauri submodule initialized?`
    );
  }

  const modules = readdirSync(permissionsDir, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name)
    .filter((name) => existsSync(join(permissionsDir, name, 'autogenerated', 'reference.md')))
    .sort();

  if (modules.length === 0) {
    throw Error(
      `Could not find any autogenerated permission references in ${permissionsDir}. Is the Tauri submodule initialized?`
    );
  }
...
---`,
    ...
    modules.map((module) => `- \`core:${module}:default\``).join('\n'),
  ];

  for (const module of modules) {
    const reference = readFileSync(
      join(permissionsDir, module, 'autogenerated', 'reference.md'),
      'utf-8'
    );

    const [head, table] = reference.split(/^## Permission Table$/m);
    if (table === undefined) {
    ...
    if (!defaults || defaults.length === 0) {
      throw Error(
        `Could not find the default permission set in the ${module} permission reference`
      );
    }
    ...
    output.push(
      ...
      '### Default Permission',
      `The default permission, \`core:${module}:default\`, includes the following:`,
      defaults.join('\n'),
      '### Permission Table',
      table.trim().replaceAll(/(Enables|Denies) the ([a-z0-9_]+) command/g, '$1 the `$2` command')
    );
  }
