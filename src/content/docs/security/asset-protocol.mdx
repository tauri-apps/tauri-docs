...
For globs that should match **files** under a tree, prefer **`**/_`** (and variants like `$DIR/\*\*/_`) rather than bare `**`, consistent with other Tauri path examples. Bare `**` is easy to misuse when you intend “everything under this directory recursively.”
...
| Broad `**` seems wrong                                              | Try **`**/_`** for file-oriented globs; see [Embedding Additional Files](/develop/resources/) for similar `**`vs`**/_` guidance in bundle resources.                                                                                           |
