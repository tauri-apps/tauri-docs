...
Tauri can serve files from disk into the WebView through the **asset** custom protocol (for example when you use [`convertFileSrc`](/reference/javascript/api/namespacecore/#convertfilesrc) in the frontend). Whether a path is allowed is controlled by **`app.security.assetProtocol`** in `tauri.conf.json`.
