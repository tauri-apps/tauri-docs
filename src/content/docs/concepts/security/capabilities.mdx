...
[references](/references/v2/config) section.
...
On Linux, Tauri is unable to distinguish between requests from an embedded `<iframe>` and the window itself.
...
inside the `capabilities/schemas` directory, which are platform-specific.
Usually you will set it to `./schemas/desktop-schema.json` or
`./schemas/mobile-schema.json` though you can also define a capability
