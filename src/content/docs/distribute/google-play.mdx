...
Note that Tauri uses an Android Studio project under the hood, so any official practice for
...
After creating a Play Console developer account, the first step to distribute your Tauri app in Google Play
is to register your app in Google Play Console website, which will guide you through all the required forms and setup tasks.
...
```
tauri android build --aab --target aarch64 --target armv7
```
...
Note that this is only useful for testing or distribution outside Google Play, as it reduces the file size but is less convenient to upload
(Google Play handles the supported architectures for you).
