...
:::note
Just to note the `cargo-tauri.hook` is only avalible in unstable channel.
:::

Using the `rustPlatform.buildRustPackage` you can use the `cargo-tauri.hook` to initialize the building process.

For more info check the [Nixpkgs Reference Manual](https://nixos.org/manual/nixpkgs/unstable/#tauri-hook)
