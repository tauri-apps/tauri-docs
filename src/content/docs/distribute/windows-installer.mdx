...
Save the fragment file with the `.wxs` extension in the `src-tauri/windows/wix-installer-fragments` folder and reference it on `tauri.conf.json`:
...
```json
{
  "bundle": {
    "windows": {
      "wix": {
        "fragmentPaths": ["./windows/wix-installer-fragments/registry.wxs"],
...
here are a few [Tauri-specific translations] required, so if you see untranslated texts feel free to open a feature request in [Tauri's main repo].
...
This is only supported for NSIS installers.
