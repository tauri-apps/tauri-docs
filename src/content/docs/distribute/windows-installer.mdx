...
Tauri includes the hook file from its generated NSIS script. At the top level of a hook file, `${__FILEDIR__}` is the hook file's directory, so you can include a file beside it directly. NSIS expands hook macros in the generated script. Save the directory before defining a macro if the macro needs a file beside the hook:
