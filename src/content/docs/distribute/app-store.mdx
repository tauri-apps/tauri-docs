...
App Store is the app marketplace maintained by Apple.
You can distribute your Tauri app targetting macOS and iOS.
...
```
tauri icon /path/to/app-icon.png --ios-color #fff
```
...
Note that the value provided in the _Bundle ID_ field must match the identifier defined in [`tauri.conf.json > identifier`].
...
Then merge that config file with the main one when building your Tauri app for App Store:

<CommandTabs
  npm="npm run tauri build --bundles app --target universal-apple-darwin --config src-tauri/tauri.appstore.conf.json"
  yarn="yarn tauri build --bundles app --target universal-apple-darwin --config src-tauri/tauri.appstore.conf.json"
  pnpm="pnpm tauri build --bundles app --target universal-apple-darwin --config src-tauri/tauri.appstore.conf.json"
  cargo="cargo tauri build --bundles app --target universal-apple-darwin --config src-tauri/tauri.appstore.conf.json"
/>
...
To create a new API key, open the [App Store Connect's Users and Access page], select the Keys tab, click on the Add button and select a name and the Developer access.
The APPLE\*API_ISSUER (Issuer ID) is presented above the keys table, and the APPLE_API_KEY_ID is the value on the Key ID column on that table.
