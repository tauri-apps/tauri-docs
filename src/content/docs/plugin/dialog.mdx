...
Shows a message dialog with an `Ok` button. `message` resolves with the button that was pressed: `'Ok'`, `'Yes'`, `'No'`, `'Cancel'`, or the label of a custom button.
...
The `buttons` option switches the dialog to two or three buttons, either a platform default set (`'OkCancel'`, `'YesNo'`, `'YesNoCancel'`) or custom labels:
