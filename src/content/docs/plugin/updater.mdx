...
On the Rust side you can either change the default target globally when initializing the plugin or via the updater builder:
...
The Rust API also allows the updater to be configured at runtime for more flexibility.

- `endpoints` and `pubkey`:
  ...
  let update = app
    ...
    // setting the pubkey at runtime is optional, but can be done if you have custom key rotation logic
    .pubkey("<insert public key here>")
    // by default Tauri does not allow downgrades and only
    .version_comparator(|current, update| {
      true
      // default: `update.version > current`
    })
    .build()?
    .check()
    .await?;
