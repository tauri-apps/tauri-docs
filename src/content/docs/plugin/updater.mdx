...
Automatically update your tauri app with a update server or a static JSON.
...
While building your update artifacts you need to have the private key in your ENVIROMENT varibales.
...
Setup the `tauri.config.json` in this format for the updater to start working.

| Keys           | Description                                                                            |
| -------------- | -------------------------------------------------------------------------------------- |
| `pubkey`       | This has to be the public key generated from the tauri cli in the step above.          |
| `endpoints`    | This must be an array of enpoint URLs as strings. TLS is enforced in production mode.  |
...
<Tabs>
  ...
  When using static you just need to return a JSON containing the reqired information.
