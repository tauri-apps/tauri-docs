...
On Linux, Tauri will create the normal AppImage, Deb, and RPM bundles inside the `target/release/bundle/appimage/`, `deb/` and `rpm/` folders:
...
- `myapp.deb` - The standard Deb package. It will be re-used by the updater.
- `myapp.deb.sig` - The signature of the update bundle.
- `myapp.rpm` - The standard RPM package. It will be re-used by the updater.
- `myapp.rpm.sig` - The signature of the update bundle.

The updater installs the same format the app was installed with: AppImage updates in place, while Deb and RPM updates install the new package. Installing a Deb or RPM update requires root, so the updater prompts the user for elevation: `pkexec` if available, falling back to a graphical `sudo` prompt (`zenity` or `kdialog`) and then to terminal `sudo`. AppImage updates need no elevation. Deb updates require updater plugin 2.1.0, RPM updates 2.10.0.
...
| `dangerousAcceptInvalidCerts`        | Setting this to `true` makes the updater accept invalid TLS certificates, e.g. for an internal update server with a self-signed certificate. Use this configuration with caution!                                                                                                             |
| `dangerousAcceptInvalidHostnames`    | Setting this to `true` makes the updater accept TLS certificates whose hostname does not match the endpoint. Use this configuration with caution!                                                                                                                                             |
...
- `{{arch}}`: The architecture of the machine (one of `x86_64`, `i686`, `aarch64`, `armv7` or `riscv64`).
- `{{bundle_type}}`: The format the running app was installed with (one of `appimage`, `deb`, `rpm`, `app`, `msi` or `nsis`; `unknown` if it cannot be detected), letting the server pick the matching package.
...
| `platforms` | Each platform key is in the `OS-ARCH` format, where `OS` is one of `linux`, `darwin` or `windows`, and `ARCH` is one of `x86_64`, `aarch64`, `i686`, `armv7` or `riscv64`. |
...
A platform key may also carry an install-format suffix in the `OS-ARCH-FORMAT` format, where `FORMAT` is one of `appimage`, `deb`, `rpm`, `app`, `msi` or `nsis`. The updater first looks for the key matching its own install format and falls back to the plain `OS-ARCH` key. This is how you serve different packages when shipping several Linux formats, e.g. `linux-x86_64-deb` and `linux-x86_64-rpm` alongside `linux-x86_64` for the AppImage.
...
The required keys are `"url"`, `"version"` and `"signature"`; the others are optional. The server may include additional fields of its own: the plugin does not parse them, but the app can read them through `update.rawJson` in JavaScript or `update.raw_json` in Rust.
...
When checking for updates it is possible to define a custom request timeout, a proxy and request headers. The proxy and headers are reused for the download step, but the timeout is not: `download` and `downloadAndInstall` accept their own `timeout` (and replacement `headers`) in a separate options argument.
...
if (update) {
  // the download options are passed separately
...
To allow downgrades, pass the `allowDowngrades` option to `check`. Any version different from the current one is then accepted as an update:
...
To allow downgrades, use the updater builder's `version_comparator` API:
