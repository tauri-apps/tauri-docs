...
:::note[Signing environment variables]
Bundling reads **`TAURI_SIGNING_PRIVATE_KEY`**: set it to either the **path** of your private key file or the **key contents** as a string (path is tried first; if the value is not an existing file, it is treated as the key material). Optionally set **`TAURI_SIGNING_PRIVATE_KEY_PASSWORD`** if the key is encrypted.

`tauri signer generate` may print a `TAURI_SIGNING_PRIVATE_KEY_PATH` hint. For `tauri build` / bundling, put that path (or the key string) in `TAURI_SIGNING_PRIVATE_KEY` — there is no separate path-only variable used by the bundler.
:::

<Tabs>
  <TabItem label="Mac/Linux">
  ```sh frame=none
  # Path to the key file (preferred) or the key contents as a string
  ...
  ```ps frame=none
  # Path to the key file (preferred) or the key contents as a string
  $env:TAURI_SIGNING_PRIVATE_KEY="$env:USERPROFILE\.tauri\myapp.key"
  <# optionally also add a password #>
