...
**Note**: the JavaScript `relaunch()` doesn't currently work on macOS apps, please use the Rust `restart()` command instead. Additionally, make sure you add an explicit `window.show()` on app startup, if you want `restart()` to show the app again after restarting.
