...
<Tabs>
  ...
  <TabItem label = "Manual">
    ...
    3. <TutorialStep tutorial="plugin-log" step="add-js-bindings">
...
With the plugin registered with Tauri during the [setup](#setup) above, all the plugin's
APIs are available through the JavaScript guest bindings:
...
<TutorialStep tutorial="plugin-log" step="usage-js">
...
<TutorialStep tutorial="plugin-log" step="permissions">
  Until the capability grants the permission, the log command is denied by the
  ACL; with `log:default` granted it succeeds:
</TutorialStep>
