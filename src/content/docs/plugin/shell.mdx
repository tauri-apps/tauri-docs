...
The legacy `open` command is still available for compatibility. It is controlled by `tauri.conf.json > plugins > shell > open`: `true` or leaving it unset applies the default validation regex (`^((mailto:\w+)|(tel:\w+)|(https?://\w+)).+`), a string registers a custom regex, and `false` disables the command for JavaScript entirely.
