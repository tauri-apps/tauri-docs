...
1. Start with creating `menu_plugin.rs` in `src-tauri/src` and paste there this code:
...
2. Create `tray.rs` in the same directory with code featured below.
    This code features basic menu.
...
pub fn create_tray<R: Runtime>(app: &tauri::AppHandle<R>) -> tauri::Result<()> {
    ...
    let menu = Menu::with_items(app, &[&toggle_i, &new_window_i, &quit_i])?;
...
3. In `.setup()` in your `lib.rs` file initialize `tray` and `menu`:

```rust title="src-tauri/src/lib.rs" ins={1,6-26}
...
```rust title="src-tauri/src/lib.rs" ins={4-14}
pub fn run() {
    tauri::Builder::default()
        // ..
...
If you want to create more complicated menu here is an example of menu, that can switch between two states,
switch between tray icons, and remove iself.
