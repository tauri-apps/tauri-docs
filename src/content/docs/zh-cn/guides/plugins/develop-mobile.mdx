...
如果你有一个现有的插件，并且想添加安卓或 iOS 的功能，你可以使用 `plugin android add` 和 `plugin ios add`来添加构建移动端插件项目，并指导你完成所需的更改。
...
一个安卓 Tauri 插件的定义为一个扩展了 `app.tauri.plugin.Plugin` 并标注 `app.tauri.annotation.TauriPlugin` 的 Kotlin 类。每个被标注为 `app.tauri.annotation.Command` 的方法都可以被 Rust 或 JavaScript 调用。
...
辅助函数可以通过使用 [`addPluginListener`](/references/v2/js/core/namespaceprimitives/#addpluginlistener) 函数在 NPM 包中找到并使用。
