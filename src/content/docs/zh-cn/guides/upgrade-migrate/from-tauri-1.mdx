...
- `api::process::Command`，`tauri::api::shell` 和 `tauri::Manager::shell_scope` APIs 被移除。使用 `tauri-plugin-shell` 作为替代。[迁移](#迁移到-shell-插件)
...
- `scope::IpcScope` 被移除。使用 `scope::ipc::Scope` 作为替代。
- `scope::FsScope`，`scope::GlobPattern` 和 `scope::FsScopeEvent` 被移除。使用 `scope::fs::Scope`，`scope::fs::Pattern` 和 `scope::fs::Event` 作为替代。
...
- `emit_filter` 现在基于 [`EventTarget`](https://docs.rs/tauri/2.0.0-beta/tauri/event/enum.EventTarget.html) 进行过滤，而不是基于一个视窗。
...
`tauri::Builder::menu` 现在接收一个闭包，因为菜单需要一个 `Manager` 实例来构造。参考[相关文档](https://docs.rs/tauri/2.0.0-beta/tauri/struct.Builder.html#method.menu)以获取更多信息。
...
Rust 的 `tauri::api::shell` 和 JavaScript 的 `@tauri-apps/api/shell` APIs 被移除。使用 `@tauri-apps/plugin-shell` 插件作为替代：
...
查阅 [`TrayIconBuilder`](https://docs.rs/tauri/2.0.0-beta/tauri/tray/struct.TrayIconBuilder.html) 以获取更多信息。
