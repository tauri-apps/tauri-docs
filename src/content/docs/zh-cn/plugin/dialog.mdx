...
在 iOS 上，返回的是 `file://<path>` URI。

在 Android 上，返回的是 [content URIs]。

[filesystem plugin] 可以直接处理任何路径格式。
:::
...
[filesystem plugin]: /zh-cn/plugin/file-system/
