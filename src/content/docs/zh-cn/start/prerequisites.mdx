...
为了完全支持 Tauri 和 [`trunk`](https://trunk-rs.github.io/trunk/) 等工具，请确保在安装程序对话框中的 `default host triple` 选择 MSVC Rust 工具链。根据你的系统，它应该是 `x86_64-pc-windows-msvc`、`i686-pc-windows-msvc` 或 `aarch64-pc-windows-msvc`。
