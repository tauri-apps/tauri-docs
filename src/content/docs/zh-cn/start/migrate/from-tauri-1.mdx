...
要为应用程序启用权限，您必须在 `src-tauri/capabilities` 文件夹内创建授权配置文件，随后 Tauri 会自动为您配置其他一切。

`migrate` 命令行工具命令会自动解析你的授权许可列表 v1 并生成相关的授权配置文件。

要了解有关权限和授权配置的更多信息，请参阅[安全文档](/zh-cn/security/)。
