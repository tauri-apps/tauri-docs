...
为了开始使用 Tauri 构建项目，您首先需要安装一些依赖项：
...
如果您的发行版未包含在上面，那么您可能需要查阅 [Awesome Tauri on GitHub](https://github.com/tauri-apps/awesome-tauri#guides) 以获知是否已有指南被创建。
...
:::tip[提示]
WebView 2 已安装在 Windows 10（从版本 1803 开始）和更高版本的 Windows 上。如果您正在这些版本之一上进行开发，则可以跳过此步骤并直接转到 [下载并安装 Rust](#rust)。
:::
...
Tauri 使用 [Rust](https://www.rust-lang.org) 构建并需要它进行开发。使用以下方法之一安装 Rust。您可以在 https://www.rust-lang.org/tools/install 查看更多安装方法。
...
下一步：[配置移动端](#移动端配置) 如果你想要在 Android 或 iOS 上开发应用。否则前往 [创建新项目](/zh-cn/guides/create/)。

## 移动端配置

如果您想将应用适配到 Android 或 iOS，则需要安装一些其他依赖项：
...
下一步：[配置 iOS](#ios) 或 [创建新项目](/zh-cn/guides/create/).

### iOS

:::caution[仅 macOS]
iOS 开发需要 Xcode，并且仅在 macOS 上可用。确保您在 [macOS 系统依赖项部分](#macos) 中安装了 Xcode 而不是 Xcode 命令行工具。
:::
...
下一步：[创建新项目](/zh-cn/guides/create/).

## 故障排除

如果您在安装过程中遇到任何问题，请务必查看 [故障诊断指南](/zh-cn/guides/troubleshoot) 或联系 [Tauri Discord](https://discord.com/invite/tauri) 以寻求帮助。

<Card title="下一步" icon="rocket">

现在，您已经安装了所有前置要求，您可以准备好 [创建您的第一个 Tauri 应用程序](/zh-cn/guides/create/)！
