...
确保您已经熟悉[插件开发](/zh-cn/guides/plugins)中的概念，因为本指南大量使用基于那篇文章的概念。
...
按照[插件开发](/zh-cn/guides/plugins#initialize-plugin-project)中的步骤初始化新的插件项目。

如果你有一个现有的插件，并且想添加安卓或 iOS 的功能，你可以使用 `plugin android add`和`plugin ios add`来添加构建移动端插件项目，并指导您完成所需的更改。
...
一个安卓 Tauri 插件由一个扩展了 `app.tauri.plugin.Plugin` 的 Kotlin 类定义，并由 `app.tauri.annotation.TauriPlugin` 标注。 每个被标注为 `app.tauri.annotation.Command` 的方法都可以被 Rust 或 JavaScript 调用。

Tauri 使用 Kotlin 作为默认的安卓插件实现，但是您也可以使用 Java 作为替代。在创建一个插件后，在 Android Studio 中右键 Kotlin 插件类并点击菜单中的”将 Kotlin 文件转换为 Java 文件“按钮。Anroid Studio 会指导您完成到 Java 项目的迁移。
...
插件被定义为一个 [Swift 包](https://www.swift.org/package-manager/)，因此您可以使用它的包管理器以管理依赖

## 插件配置

前往插件开发指南的[插件配置](/zh-cn/guides/plugins/#plugin-configuration)章节以获取更多关于插件开发的配置信息。
...
在插件开发指南中还有一些其他[插件生命周期事件](/zh-cn/guides/plugins#lifecycle-events)的介绍。
...
使用 [`tauri::plugin::PluginHandle`](https://docs.rs/tauri/2.0.0-alpha/tauri/plugin/struct.PluginHandle.html) 以在 Rust 中创建一个移动端命令：
