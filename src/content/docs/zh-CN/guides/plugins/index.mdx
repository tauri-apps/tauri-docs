...
这个指南是用于指导开发 Tauri 插件。如果您想要寻找关于当前可用的插件的列表及其使用说明，请前往[特征和菜谱列表](/zh-cn/features)。

:::

插件可以挂载到 Tauri 的生命周期中：暴露需求 webview API 的 Rust 代码，使用 Rust、Kotlin 或 Swift 代码处理命令，以及更多……

Tauri 提供了一个基于 webview 功能的视窗系统，一个在 Rust 进程和 webview 之间发送信息的方式，一个事件系统，以及一些增强开发体验的工具。
从设计上讲，Tauri Core 并不包含所有人都需要的功能。相反，它提供了一种将外部功能添加到名为插件的 Tauri 应用程序中的机制。
...
插件名称由插件配置中的 [`tauri.conf.json > plugin`](/2/reference/config/#pluginconfig) 和许可列表中的配置确定。

默认情况下，Tauri 会在你的插件箱前面加上 `tauri-plugin-`。这有助于你的插件被 Tauri 社区发现，但不是必须的。初始化新插件项目时，必须提供其名称。生成包名称将为 `tauri-plugin-{plugin-name}`，JavaScript NPM包名称为 `tauri-plugin-{plugin-name}-api`（尽管我们建议使用[NPM范围](https://docs.npmjs.com/about-scopes)如果可能的话）。NPM 包的 Tauri 命名约定是 `@scope-name/plugin-{plugin-name}`。
...
还有一些特定的[移动端插件生命周期事件](/zh-cn/guides/plugins/develop-mobile#lifecycle-events)。
...
在这个生命周期钩子中，你会被任何[事件循环事件](https://docs.rs/tauri/2.0.0-alpha/tauri/enum.RunEvent.html)通知。
...
例如，[`global-shortcut` 插件](/zh-cn/features/global-shortcut) 定义了一个 `GlobalShortcut` 结构体，可以被 `GlobalShortcutExt` trait 的 `global_shortcut` 方法使用：
...
阅读[命令指南](/zh-cn/features/commands)以获取更多关于 Tauri 命令的信息。
...
插件可以像 Tauri 应用程序一样管理状态。阅读[状态管理指南](/zh-cn/guides/state-management)以获取更多信息。
