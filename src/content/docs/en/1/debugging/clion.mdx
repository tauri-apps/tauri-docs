...
![Add Run/Debug Configuration](./assets/clion/add-cargo-config-dark.png)
...
![Add `--no-default-features` flag](./assets/clion/set-no-default-features-dark.png)
...
![Rename Configuration](./assets/clion/rename-configuration-dark.png)

<Callout kind="caution">
  On Windows you must also make sure that CLion uses the correct debugger
  toolchain. To do this, open the Settings (`File -> Settings...`), select
  `Build, Execution, Deployment -> Toolchains` and move the `Visual Studio`
  toolchain to the top.
</Callout>
