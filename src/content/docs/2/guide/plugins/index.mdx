...
To bootstrap a new plugin project, run `plugin init` command. If you do not need the NPM package, use the `--no-api` CLI flag.
...
If you have an existing plugin and would like to add Android or iOS capabilities to it, you can use the `plugin android add` and `plugin ios add` commands to bootstrap the mobile library projects and guide you through the changes needed.
