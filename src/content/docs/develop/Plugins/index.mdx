...
Plugins can declare which platforms they support, and to what extent, in the `[package.metadata.platforms.support]` section of the crate's `Cargo.toml`:
...
- `level` (required): `"full"` if the plugin works as intended, `"partial"` if it works with limitations, or `"none"` if the platform isn't supported.
- `notes` (optional): a short description of the caveats or limitations. It renders as Markdown on the plugin's page. In the [support table](/plugin/#support-table) it shows up as plain text in a tooltip.

The support table and the platform filter on the [Features & Recipes](/plugin/) page are generated from this metadata.
