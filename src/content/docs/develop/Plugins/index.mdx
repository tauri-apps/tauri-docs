...
Plugins can declare which platforms they support, and to what extent in `[package.metadata.platforms.support]` section of the crate's `Cargo.toml`:
...
Each entry maps a platform (`windows`, `linux`, `macos`, `android`, `ios`) to a support declaration:

- `level`: one of `"full"` (works as intended on that platform), `"partial"` (works with limitations) or `"none"` (not supported).
- `notes`: a free-form string describing caveats or limitations, shown alongside the support level. Markdown is supported.

This metadata is the source of the platform [support](/plugin/#support-table) and the plugins search. Declaring the metadata makes your plugin's platform support discoverable in the published crate and available to future tooling, so we encourage including it.
