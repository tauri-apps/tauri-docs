...
For example, the [`global-shortcut plugin`](/feature/global-shortcut) defines a `GlobalShortcut` struct that can be read by using the `global_shortcut` method of the `GlobalShortcutExt` trait:
...
Your plugin consumer can define a scope for a specific command in their capability file (see the [documentation](/reference/acl#scopes)).
...
See the [Access Control List](/reference/acl) documentation for more information.
