...
Tauri plugins have a prefix (`tauri-plugin-` prefix for the Rust crate name and `@tauri-apps/plugin-` for the NPM package) followed by the plugin name. The plugin name is specified on the plugin configuration under [`tauri.conf.json > plugin`](/references/config/#pluginconfig) and on the allowlist configuration.
...
Your plugin consumer can define a scope for a specific command in their capability file (see the [documentation](/references/acl#scopes)).
...
See the [Access Control List](/references/acl) documentation for more information.
