...
Visual Studio Windows Debugger is faster and has better support for enums than [`vscode--lldb`] on Windows
  ...
  "configurations": [
    {
      ...
      "type": "cppvsdbg",
      "request": "launch",
      // change the exe name to your actuall exe name
      // (to debug release build, change `target/debug` to `release/debug`)
...
`tasks.json` is the same as with lldb, except you need to add a config group and target your `preLaunchTask` from `launch.json` to it if you want it to always compile before launching
