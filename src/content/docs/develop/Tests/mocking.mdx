...
<SinceVersion version="2.6.3"/>

There is partial support of the [Event System] to simulate events emitted by your Rust code:
...
test("mocked event", () => {
    // a call to `mockIPC` is still required even if no `invoke` calls are mocked
    mockIPC(() => { }); 
