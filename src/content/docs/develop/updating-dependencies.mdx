...
Since the JavaScript APIs rely on the Rust code in the backend, when we add a new feature, you need to upgrade both side to make it work, so please make sure you have the same minor version of npm package `@tauri-apps/api` and cargo crate `tauri` synced, this is also needed for the plugins, for example, you need to sync the minor version of npm package `@tauri-apps/plugin-fs` and cargo crate `tauri-plugin-fs`
