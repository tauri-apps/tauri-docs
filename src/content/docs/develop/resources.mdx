...
To learn about where does `$RESOURCE` resolve to on each platforms, see the documentation of [`resource_dir`]
...
we use a special URI prefix `asset://localhost/` here that can be used with the [`plugin-fs`],
...
If you want or must need the resource files to be on a real file system, copy the contents out manually through the [`plugin-fs`]
...
To use it with the [`plugin-opener`]:
