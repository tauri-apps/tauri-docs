...
To learn about where does `$RESOURCE` resolve to on each platforms, see the documentation for [`resource_dir`]
...
with that, you can read the files through [`FsExt::fs`]
like this: `app.fs().read_to_string(app.path().resource_dir().unwrap().join("resource"));`

You can copy the contents out manually if you need them to be on a real file system
