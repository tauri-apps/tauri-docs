...
`resources` expects a list of strings targeting files or directories either with absolute or relative paths. It supports glob patterns (bash-style) in case you need to include multiple files from a directory.
...
The syntax for resource source paths works like this:

- `"dir/file.txt"`: Will copy the `file.txt` file into the target resource directory.
- `"dir/*"` / `dir/**`: Will copy all files in the `dir` directory _non-recursively_ (sub-directories will be ignored) into the target resource directory.
- `"dir/**/*"`: Will copy all files in the `dir` directory _recursively_ (all files in all sub-directories) into the target resource directory. Note, this does **not** include the files from `dir/*`, but only those in sub-directories.
- `"dir/"`: Will copy all files **and directories** _recursively_ into the target resource directory. Use this if you also want to preserve the file system structure of your files and directories.

The `target resource directory` is either the right value of the object notation, or a reconstruction of the source path in the array notation.
