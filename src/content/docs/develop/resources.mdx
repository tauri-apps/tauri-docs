...
- `"dir/*"`: Will copy all files in the `dir` directory _non-recursively_ (sub-directories will be ignored) into the target resource directory.
- `"dir/**`: Will throw an error because `**` only matches directories and therefore no files can be found.
- `"dir/**/*"`: Will copy all files in the `dir` directory _recursively_ (all files in `dir/` and all files in all sub-directories) into the target resource directory.
- `"dir/**/**`: Will throw an error because `**` only matches directories and therefore no files can be found.
