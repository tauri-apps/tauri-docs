...
:::note[Git dependency during the alpha]

`tauri-runtime-cef` is not published to crates.io yet because it depends on a Git fork of `winit`. Until it is, depend on it from the Tauri repository and patch the other Tauri crates to the same source, so that a single copy of each crate is compiled:

```toml title="src-tauri/Cargo.toml"
[dependencies]
tauri-runtime-cef = { git = "https://github.com/tauri-apps/tauri", branch = "v3" }

[patch.crates-io]
tauri = { git = "https://github.com/tauri-apps/tauri", branch = "v3" }
tauri-build = { git = "https://github.com/tauri-apps/tauri", branch = "v3" }
```
...
Set `bundle > cef > embed` to `false` for an app that loads CEF at run time from a shared, machine-wide runtime outside its own bundle. The app locates that runtime through the `TAURI_CEF_LIBRARY_PATH` environment variable.
The framework, `libcef`, and their resources are then left out of every bundle, and the app must find a runtime at launch or it does not start. On macOS the helper apps are still produced, since they belong to the app rather than to the distribution.
