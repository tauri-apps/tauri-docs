...
In the example below `bundle` is merged, while `bundle.resources` is replaced.
This also applies to `app.windows`, even though its entries are objects: a platform-specific entry
replaces the base entry instead of being merged into it, so any field you omit falls back to its default
rather than the value in your base configuration.
Repeat every field you want to keep.
:::
