...
:::note
Objects are merged key by key, but arrays are replaced as a whole, including arrays nested in objects.
In the example below `bundle` is merged, while `bundle.resources` is replaced: the platform-specific
entries do not extend the base list, they take its place.
The same holds when the array elements are objects, such as `app.windows`. A platform-specific entry
replaces the base entry rather than merging into it, so any field you omit falls back to its default
instead of the value in your base configuration.
