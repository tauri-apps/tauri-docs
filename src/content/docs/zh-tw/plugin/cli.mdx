...
1. 通过将以下内容新增到 `Cargo.toml` 的文件中来安装外掛。
...
标志参数是一个独立的键，它的存在或不存在为应用程序提供資訊。通过以下配置：
...
| `cli:allow-cli-matches` | 在没有预先配置有效範圍的情况下，启用 cli_matches 命令。 |
| `cli:deny-cli-matches`  | 拒绝没有预先配置有效範圍的 cli_matches 命令。           |
