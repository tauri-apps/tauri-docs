...
<Tabs>
    ...
    1. 运行 `cargo add tauri-plugin-http` 命令，将外掛新增到项目的 `cargo.toml` 依赖中。
...
http 外掛既有 JavaScript API 版本，也有 Rust [reqwest](https://docs.rs/reqwest/) 重新导出的版本。
...
1. 配置允许访问的 URL
   ...
   更多資訊，请参阅[权限概述](/zh-tw/security/permissions/)的文档。
...
在 Rust 中，你可以利用外掛重新导出的 `reqwest` 包。更多细节请参考 [reqwest 文档](https://docs.rs/reqwest/)。
