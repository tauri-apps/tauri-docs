...
1. 运行 `cargo add tauri-plugin-deep-link` 以将外掛新增到 `Cargo.toml` 的项目依赖中。
...
其中 `$APP_BUNDLE_ID` 是 `tauri.conf.json > identifier` 上定义的值，其中 `-` 替换为 `_` ， `$CERT_FINGERPRINT` 是你的应用签名证书的 SHA256 指纹列表，请参阅 [verify android applinks](https://developer.android.com/training/app-links/verify-android-applinks#web-assoc) 以获取更多資訊。
...
其中 `$DEVELOPMENT_TEAM_ID` 是 `tauri.conf.json > tauri > bundle > iOS > developmentTeam` 或 `TAURI_APPLE_DEVELOPMENT_TEAM` 环境变量上定义的值，`$APP_BUNDLE_ID`是定义在`tauri.conf.json > identifier` 上的值。请参阅 [applinks.details](https://developer.apple.com/documentation/bundleresources/applinks/details) 以获取更多資訊。

#### App

你还需要将相关域名新增到应用的 `entitlement` 文件中。
...
有关更多資訊，请参阅 [supporting associated domains](https://developer.apple.com/documentation/xcode/supporting-associated-domains?language=objc)。
...
| `deep-link:allow-get-current` | 在没有预先配置的有效範圍的情况下启用 get_current 命令。 |
