...
<Tabs>
    ...
    1. 运行 `cargo add tauri-plugin-dialog` 命令，将外掛新增到项目的 `cargo .toml` 依赖中。
...
对话框外掛可以在 JavaScript 和 Rust 中使用。你可以这样使用它：
...
可以在 JavaScript API 参考中查看所有 [Dialog 选项](/zh-tw/reference/javascript/dialog/)。
