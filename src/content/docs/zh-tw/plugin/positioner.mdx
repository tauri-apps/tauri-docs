...
这个外掛是 [electron-positioner](https://github.com/jenslind/electron-positioner) 的 Tauri 版本。
...
:::note
如果你只想从 Rust 代码中移动窗口，你只需要 `src-tauri/Cargo.toml` 中的依赖，并且如果你选择自动安装，可以从 `lib.rs` 中删除外掛注册。
:::
...
<Tabs>
    ...
    <TabItem label = "手动">
    	<Steps>
    	1. 在你的 `Cargo.toml` 文件中新增以下内容来安装定位器外掛。
...
外掛的 API 可以通过 JavaScript Guest 绑定获得。
...
| 权限                           | 描述                                                  |
| ------------------------------ | ----------------------------------------------------- |
| `positioner:allow-move-window` | 在没有预先配置的有效範圍的情况下启用 move_window 命令。 |
