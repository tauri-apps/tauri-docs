...
<Tabs>
    ...
    1. 运行 `cargo add tauri-plugin-os` 命令，将外掛新增到项目的 cargo .toml 依赖中。
...
通过这个外掛，您可以查询当前操作系统的多个資訊。请参阅 [JavaScript API](/zh-tw/reference/javascript/os/) 或 [Rust API](https://docs.rs/tauri-plugin-os/) 参考资料中的所有可用函数。
