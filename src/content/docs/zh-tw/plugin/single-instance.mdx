...
使用单实例外掛确保 Tauri 应用程序在同一时间只运行单个实例。
...
<Tabs>
    ...
    <ol>
      ...
      <li>
        **初始化：** 更新 `lib.rs` （或者只针对桌面应用的 `main.rs`） 来初始化外掛。
...
外掛已经安装并初始化，应该可以立即正常运行。尽管如此，我们也可以使用 `init()` 方法来增强它的功能。

外掛的 `init()` 方法接受一个闭包，该闭包在新 App 实例启动时调用，但由外掛关闭。
这个闭包有三个参数：
