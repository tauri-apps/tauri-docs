...
查看[对比](https://www.digicert.com/difference-between-dv-ov-and-ev-ssl-certificates)了解更多关于 OV 和 EV 证书的資訊。
:::
...
要让 Windows 为代码签名做好准备，我们必须做几件事。这包括将我们的证书转换为特定的格式，安装此证书，以及从证书中解码所需的資訊。
...
2. 在 `tauri.conf.json` 文件中，你需要找到 `tauri` -> `bundle` -> `windows` 部分。我们已经获取了三个变量的資訊。请按照下面的示例填写它们。
...
我们需要新增一些 GitHub 密钥来正确配置 GitHub Action。你可以随意命名它们。

- 你可以查看 [encrypted secrets] 指南，了解如何新增 GitHub 密钥。
...
1. 我们需要在工作流中新增一个步骤来将证书导入 Windows 环境。该工作流完成以下工作。
...
3. 就在上面 `-name: install app dependencies and build it` 你需要新增以下步骤
