...
由于 Tauri 中的所有移动部分，您可能会遇到需要调试的问题。打印错误详细資訊的位置很多，Tauri 包含一些工具，使调试过程更加简单。

## 仅开发模式的代码

工具包中最有用的调试工具之一是能够在代码中新增调试语句。但是，您通常不希望这些程序最终进入生产环境，这就是检查您是否在开发模式下运行的功能派上用场的地方。
...
有时你的 Rust 代码中可能有错误，而 Rust 编译器可以给你很多資訊。例如，如果 `tauri dev` 崩溃，您可以在 Linux 和 macOS 上像这样重新运行它：
...
此命令为您提供精细的堆栈跟踪。一般来说，Rust 编译器会为您提供有关问题的详细資訊，例如：
...
[在 VS Code 调试]: /zh-tw/develop/debug/vscode/
