...
在 Tauri 中的所有动态部分，您可能会遇到需要调试的问题。有很多位置会打印错误详细資訊，Tauri 还包含一些工具来简化调试过程。
...
```rust
println!("来自 Rust 的資訊: {}", msg);
```

有时，您的 Rust 代码中可能存在错误，Rust 编译器可以为您提供大量資訊。例如，如果 `tauri dev` 崩溃，您可以在 Linux 和 macOS 上像这样重新运行它：
...
此命令为您提供了详细的堆栈跟踪。一般来说，Rust 编译器会通过为您提供有关该问题的大量資訊来帮助您，例如：
...
你还可以从终端运行已构建的应用程序，查看 Rust 编译器注释（在出错的情况下）或你的 `println` 資訊。浏览到 `src-tauri/target/(release|debug)/[应用程序名称]`文件，然后直接在控制台中运行，或双击文件系统中的可执行文件（注意：使用此方法时，控制台会在出错时关闭）。
