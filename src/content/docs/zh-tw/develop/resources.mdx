...
要打包你需要的文件，你可以在 `tauri.conf.json` 文件中的 `bundle` 对象中新增 `resources` 属性。
...
为此，我们像上面展示的那样，在 `resources` 中新增 `"lang/*"`。
...
欲了解更多資訊，请阅读 [Scope Permissions] 查看其他选项，或查阅 [Scopes] 以获取更精细的控制。
