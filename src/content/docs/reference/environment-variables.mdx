...
- `TAURI_SIGNING_PRIVATE_KEY` — Private key used to sign updater / bundle artifacts. Accepts a **filesystem path** or the **key contents** as a string (if the value is not an existing file, it is treated as key material). This is the variable `tauri build` / the bundler reads.
- `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` — Password for `TAURI_SIGNING_PRIVATE_KEY` when the key is encrypted (optional).
