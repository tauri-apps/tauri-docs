...
Tauri apps can have [tray-type interfaces](/guides/plugin/system-tray). They can be [updated](/guides/plugin/updater/) and are managed by the user's operating system as expected. They are very small because they use the OS's webview. They do not ship a runtime since the final binary is compiled from Rust. This makes the [reversing of Tauri apps not a trivial task](/security/).
