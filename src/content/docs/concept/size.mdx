...
- [incremental:][profile-incremental] Compile your binary in smaller steps.
- [codegen-units:][profile-codegen-units] Speeds up compile times at the cost of compile time optimizations.
- [lto:][profile-lto] Enables link time optimizations.
- [opt-level:][profile-opt-level] Determines the focus of the compiler. Use `3` to optimize performance, `z` to optimize for size, and `s` for something in-between.
...
The tips on this page shrink your app's binary, but what users actually download also depends on how the app is packaged.
...
The same applies to your UI framework: if it shows up prominently in your bundle analysis, lighter alternatives such as [Preact] (a drop-in replacement for React) or compiler-based frameworks like [Svelte] and [Solid] can shave a substantial amount off your bundle.
...
[profile-incremental]: https://doc.rust-lang.org/cargo/reference/profiles.html#incremental
