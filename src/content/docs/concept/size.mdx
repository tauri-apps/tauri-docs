...
- **[`rollup-plugin-visualizer`](https://github.com/btd/rollup-plugin-visualizer)** - A tool that generates beautiful (and insightful) graphs from your Vite, Rollup, or Rolldown bundle. Very convenient for figuring out what JavaScript dependencies contribute to your final bundle size the most.

- **[`vite-bundle-analyzer`](https://github.com/nonzzz/vite-bundle-analyzer)** - You noticed a dependency included in your final frontend bundle, but you are unsure why? `vite-bundle-analyzer` renders an interactive treemap of your bundle so you can drill down into what ends up in it.

These are just a couple of tools that you might use. Make sure to check your frontend bundler's plugin list for more!
...
For a detailed explanation of each option and a bunch more, refer to the [Cargo book's Profiles section](https://doc.rust-lang.org/cargo/reference/profiles.html).
...
You can choose from a plethora of JavaScript bundlers; the de-facto standard today is [Vite](https://vite.dev), which most Tauri project templates use and which is powered by the Rust-based [Rolldown](https://rolldown.rs) bundler since Vite 8. [webpack](https://webpack.js.org) and [Rollup](https://rollupjs.org) are still common in existing projects, with Rollup's role progressively being taken over by Rolldown, its successor. All of them can produce minified JavaScript if configured correctly, so consult your bundler documentation for specific options. Generally speaking, you should make sure to:
...
Minification removes unnecessary whitespace, shortens variable names, and applies other optimizations. Most bundlers enable this by default — Vite, for example, minifies production builds out of the box. A notable exception is Rollup, where you need plugins like [@rollup/plugin-terser](https://github.com/rollup/plugins/tree/master/packages/terser).

Note: You can use minifiers like [terser](https://terser.org) and [esbuild](https://esbuild.github.io) as standalone tools.

### Disable source maps

Source maps provide a pleasant developer experience when working with languages that compile to JavaScript, such as [TypeScript](https://www.typescriptlang.org). As source maps tend to be quite large, you must disable them when building for production — most bundlers, including Vite, already do this by default. They have no benefit to your end-user, so it's effectively dead weight.
...
You can choose between a variety of manual options ([GIMP](https://www.gimp.org), [Photoshop](https://www.adobe.com/de/products/photoshop.html), [Squoosh](https://squoosh.app)) or plugins for your favorite frontend build tools ([vite-imagetools](https://github.com/JonasKruckenberg/imagetools), [vite-plugin-image-optimizer](https://github.com/FatehAK/vite-plugin-image-optimizer)).

Do note that the `imagemin` library many older plugins use is [officially unmaintained](https://github.com/imagemin/imagemin/issues/385), so prefer plugins built on actively maintained tools such as [Sharp](https://sharp.pixelplumbing.com) and [SVGO](https://svgo.dev).
...
In a Web Environment, you are supposed to use [Responsive Images](https://developer.mozilla.org/en-US/docs/Web/HTML/Guides/Responsive_images) to load the correct image size for each user dynamically. Since you are not dynamically distributing images over the web, using Responsive Images only needlessly bloats your app with redundant copies.
