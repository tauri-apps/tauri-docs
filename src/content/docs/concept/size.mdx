...
:::note
Capabilities added at runtime through the `dynamic-acl` feature (the `add_capability` API) are not visible at build time, so commands they rely on may get removed. Test your app thoroughly when using this
:::

:::note
For commands from inlined plugins, add `#![plugin(<plugin_name>)]` inside your `tauri::generate_handler![]` usage so their commands can be tracked
:::
