...
You can choose from a plethora of JavaScript bundlers; popular choices are [Vite](https://vite.dev), [webpack](https://webpack.js.org), and [Rollup](https://rollupjs.org). All of them can produce minified JavaScript if configured correctly, so consult your bundler documentation for specific options. Generally speaking, you should make sure to:
...
Minification removes unnecessary whitespace, shortens variable names, and applies other optimizations. Most bundlers enable this by default — [Vite](https://vite.dev), for example, minifies production builds with [esbuild](https://esbuild.github.io) out of the box. A notable exception is [Rollup](https://rollupjs.org), where you need plugins like [@rollup/plugin-terser](https://github.com/rollup/plugins/tree/master/packages/terser).
