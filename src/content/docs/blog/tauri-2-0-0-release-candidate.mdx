---
title: Tauri 2.0 Release Candidate Announcement
date: 2024-08-01
authors: [tweidinger]
...
A simplified [TL;DR](#Too-Long-Didn't-Read) can be found at the end of this post.
...
Another reason is that we made the mistake of overpomising this major version with "*mobile as a first class citizen*" and realized over the past months that we
...
This causes us to say that we don't want to raise expectations that Tauri 2.0 will be _the_ "*mobile as a first class citizen*" release but we want to make clear that you can develop production ready mobile applications with Tauri **NOW**.
...
We plan to release the stable version for 2.0 in the end of August. This will, at the time of writing, allow for a ~4 week release candiate cycle.
...
Feature parity and plugin development will be aligned with major versions of Tauri but will be mostly independent from Tauri core features and happen in our [plugin-workspace](https://github.com/tauri-apps/plugins-workspace) repostory.
...
You should be able to automatically migrate from the latest beta to release candidate with:
...
:::tip[Update the CLI first]
For this to succeed you must be sure to be on the latest (RC not beta) version of the Tauri [CLI](https://github.com/tauri-apps/tauri/releases?q=tauri-cli&expanded=false). If your are still on beta you can update with the CLI `cargo install tauri-cli` or `npm update tauri-cli`.
:::
...
We concluded that this overexposure will hinder us in the future and will cause unecessary breaking changes, so we decided that going from beta to RC will be the last chance for us to implement this until we start down the road of Tauri 3.0.
...
With the release candidate we added the full report to our repository. Please take your time to read the report and learn more about the awesome work of [@gronke](https://github.com/gronke) and [@pcwizz](https://github.com/pcwizz) from [RadicallyOpenSecurity](https://www.radicallyopensecurity.com/).
...
All of the above topics share a common theme. These would not have been possible without the continous support of the community, our working group and other movements working towards improving the status quo.

Before we are going to release Tauri 2.0 we want to make sure that your voices are heard, your PRs are acknowleged and the documentation is helpful for **YOU** so that you can build the next generation of cross platform apps.
