...
WebView2 は Windows 7 以降であれば利用可能で、Windows 11 にはプリインストールされています。Windows 11 より前のバージョンでは、Tauri によって生成されたインストーラーによって、WebView2 がシステムにインストールされます。

## WebKit (macOS, iOS, & Linux)

Tauri は、macOS では [WKWebView](https://developer.apple.com/documentation/webkit/wkwebview?language=objc) を利用して、Linux では `webkit2gtk` を利用して、WebKit を使用しています。

### WebKitのバージョン番号の見方

Webkit のバージョン番号は非常に複雑なので、それを理解するために役立つ情報を以下に示します。

WebKit のバージョン番号は、5 つの部分（セグメント）と、WebKit がどの OS 用にビルドされているかを示す番号（数値プレフィックス）で構成されています：

> `$(SYSTEM_VERSION_PREFIX)$(MAJOR_VERSION).$(MINOR_VERSION).$(TINY_VERSION).$(MICRO_VERSION).$(NANO_VERSION)`

先頭の番号は「`SYSTEM_VERSION_PREFIX`」（システム・バージョン識別番号）と呼ばれ、macOS と iOS ビルドにのみ存在するようです（Linux には存在しません）。さらに、最後の 2 つのセグメントが両方とも「0」の場合は、省略可能です（つまり、「`613.2.7.0.0`」のようなバージョンは「`613.2.7`」となります）。

例えば、macOS Monterey (12.x) の Safari 15.5 に同梱されている WebKit のバージョン番号は「`17613.2.7.1.8`」です。これは次のように解釈できます。
...
`SYSTEM_VERSION_PREFIX` 値への読み替え表は次のとおりです：
...
macOSでは、Tauri は macOS バージョン 10.10（Yosemite）以降にプリインストールされている WebView を使用します。これはコア・コンポーネントとみなされているため、定期的な OS アップデートで更新されます。つまり、サポートされていない macOS バージョンでは WebKit のアップデートは**受け取れません**。

あなたの利用している macOS のバージョンで `WKWebView` で使用される WebKit のバージョンを確認するには、ターミナルで次のコマンドを使用します：

```shell
awk '/CFBundleVersion/{getline;gsub(/<[^>]*>/,"");print}' /System/Library/Frameworks/WebKit.framework/Resources/Info.plist
```

#### Safari の WebKit バージョン

以下の表は、「OS バージョン」と「WebKit Safari バージョン」との対応関係を示しており、[caniuse](https://caniuse.com) などのサイトを利用して、特定の Web プラットフォーム機能がサポートされているかどうかを確認できます。

| OS Name     | OS Version                       | WebKit Version   | Safari Version | 注記                                                       |
| ----------- | -------------------------------- | ---------------- | -------------- | ---------------------------------------------------------- |
| Sonoma      | 14.0 (Beta)                      | 616.1.14.11.11   | 17.0           | 2023 M2 14" MacBook Pro にて検証済み                       |
| Ventura     | 13.4.1                           | 615.2.9.11.7     | 16.5.1         | 2023 M2 14" MacBook Pro にて検証済み                       |
|             | 13.3.1                           | 615.1.26.11.23   |                | 2023 M2 14" MacBook Pro にて検証済み                       |
|             | 13.3                             | 615.1.26.11.22   | 16.4           | 2023 M2 14" MacBook Pro にて検証済み                       |
|             | 13.2.1                           | 614.4.6.1.6      |                |                                                            |
|             | 13.2                             | ?                | 16.3           |                                                            |
|             | 13.1                             | 614.3.7.1.5      | 16.2           | 2020 M1 13" MacBook Pro にて検証済み                       |
|             | 13.0.1                           |                  |                | 2020 M1 13" MacBook Pro にて検証済み                       |
|             | 13.0                             | 614.2.9.1.12     | 16.1           | 2020 M1 13" MacBook Pro にて検証済み                       |
| Monterey    | 12.6                             |                  |                | 2020 M1 13" MacBook Pro にて検証済み                       |
|             | 12.5.1                           | 613.3.9.1.16     | 15.6.1         | 2020 M1 13" MacBook Pro にて検証済み                       |
|             | 12.5                             | [613.3.9.1.5]    | 15.6           | 2020 M1 13" MacBook Pro にて検証済み                       |
|             | 12.4                             | [613.2.7.1.8]    | 15.5           | 2020 M1 13" MacBook Pro にて検証済み                       |
...
|             | 11.3.1                           |                  |                |                                                            |
|             | 11.3                             | [611.1.21.161.3] | 14.1           | 24" M1 iMac にて WebKit 特別バージョン採用 [611.1.21.1.12] |
|             | 11.2.3                           | [610.4.3.1.7]    |                |                                                            |
|             | 11.2.2                           |                  |                |                                                            |
|             | 11.2.1                           |                  |                |                                                            |
...
|             | 11.0                             | [610.2.11.1.3]   | 14.0.1         | Safari 14.0 は iPhones でのみ利用可能                      |
...
|             | 10.15.1                          | [608.3.10.1.4]   | 13.0.3         | 2014 15" MacBook Pro にて検証済み                          |
...
|             | 10.13.2 補足アップデート         | [604.4.7.1.6]    | 11.0.2         | 27" iMac Pro に WebKit 特別バージョン採用 [604.4.7.10.6]   |
|             | 10.13.2                          | [604.4.7.1.3]    | 11.0.2         | 27" iMac Pro に WebKit 特別バージョン採用 [604.4.7.10.4]   |
