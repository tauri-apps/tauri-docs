...
Tauri plugins have a prefix (`tauri-plugin-` prefix for the Rust crate name and `@tauri-apps/plugin-` for the NPM package) followed by the plugin name. The plugin name is specified on the plugin configuration under [`tauri.conf.json > plugin`](/references/v2/config/#pluginconfig) and on the allowlist configuration.
...
To bootstrap a new plugin project, run `plugin new`. If you do not need the NPM package, use the `--no-api` CLI flag.

:::note[Tauri-Cli Beta]
These commands are for beta version of @tauri-apps/cli
:::

If you have previously installed Tauri globally, you may need to make sure your cargo and npm binaries are up to date for these commands to work as expected.
<CommandTabs
  npm="npm install -g @tauri-apps/cli@next"
...
This will initialize the plugin at the directory `tauri-plugin-[name]` and the resulting code will look like this:
...
Your plugin consumer can define a scope for a specific command in their capability file (see the [documentation](/references/v2/acl#scopes)).
...
See the [Access Control List](/references/v2/acl) documentation for more information.
