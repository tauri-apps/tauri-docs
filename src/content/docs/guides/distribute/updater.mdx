...
import updaterDialogImage from '@assets/img/guides/distribute/update-available.png';
...
`"dialog"` if present must be a boolean. By default it's set to true. If enabled, updater will be disabled as the built-in dialog handles everything. If you need custom events, you must turn off the built-in dialog.
...
{/* TODO: Update link to v2 */}
Your server can use variables defined in the [`endpoint`](https://tauri.app/v1/api/config/#updaterendpoint) url above to determine if an update is required. If you need more data, you can include additional [request headers in Rust] to your liking.
...
By default, the updater shows a dialog using Tauri's [dialog.ask](/references/v2/js/dialog/#ask) API internally. The dialog will only check for a new update when the app was just launched or when you manually [emit](/references/v2/js/core/namespaceevent/) the `"tauri://update"` event.

<div style={{ textAlign: 'center' }}>
	<Image src={updaterDialogImage} alt="Default updater dialog"/>
</div>
...
For the complete API docs see [here](/references/v2/js/updater/). An example using the JavaScript API looks like this:
