...
If you are developing in a container that shares its files with the host, you can install the flatpak on your host.
Any commands that are not labelled `(host)` must be run inside the container. 
The `(host)` commands from inside the flatpak directory on your host.
