...
8. Update your installed flatpak after building with `$ flatpak -y --user update <your flatpak id>`

If you are developing in a docker container that shares its files with the host, you can install the flatpak on your host.
To do this run commands 5 - 8 from inside the `/flatpak` directory on your host.
...
If your final binary requires more libraries than the standard tauri-app, you have to add these in your flatpak manifest.
To do this, you have to options. For fast local development, it might work to simply include the already built library file (.so) from your local system.
However, this is not recommended to use for the final build of the flatpak, because your local library file is not built for environment of the flatpak runtime. 
This might introduce various bugs which could be very hard to track down.
