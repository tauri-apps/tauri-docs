...
由于 Tauri 使用了 web 技术，这也意味着几乎所有的前端框架都与 Tauri 兼容。[前端配置指南](/zh-cn/start/frontend/)中涵盖了热门前端框架的通用配置；[渲染概念](/concepts/rendering)中讨论了哪些渲染技术（如单页应用 SPA 和服务端生成 SSG）最适合 Tauri。
