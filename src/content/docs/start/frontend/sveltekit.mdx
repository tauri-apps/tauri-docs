...
1.  ##### Disable SSR and prerendering for SPA mode

    Lastly, we need to disable SSR and prerendering by adding a root `+layout.ts` file (or `+layout.js` if you are not using TypeScript) with these contents:
    ...
    For SPA mode in Tauri, keep both `prerender = false` and `ssr = false` at the root layout. Using `prerender = true` can fail with dynamic routes (for example `src/routes/[id]`) unless every route can be statically enumerated.
