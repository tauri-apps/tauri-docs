...
Esta guía te guiará a través de la actualización de su aplicación Tauri 1.0 a Tauri 2.0.
...
Para saber más sobre el comando `migrate`, consulte la [Referencia de la interfaz de línea de comandos](/reference/cli/#migrate)
...
Las APIs de Rust `App::get_cli_matches` JavaScript `@tauri-apps/api/cli` han sido eliminadas. Utiliza en su lugar el plugin `@tauri-apps/plugin-cli`:
...
2. Úsalo en un proyecto JavaScript o Rust:
...
Las APIs Rust `App::clipboard_manager` y `AppHandle::clipboard_manager`, y la API JavaScript `@tauri-apps/api/clipboard` han sido eliminadas. En su lugar, utiliza el plugin `@tauri-apps/plugin-clipboard-manager`:
...
Las APIs de Rust `tauri::api::dialog` JavaScript `@tauri-apps/api/dialog` han sido eliminadas. Utiliza en su lugar el plugin `@tauri-apps/plugin-dialog`:
...
2. Úsalo en un proyecto de JavaScript o Rust:
...
2. Úsalo en proyectos JavaScript o Rust:
...
En el lado de Rust, la API `tauri::api::process` ha sido eliminada. Usa `tauri_plugin_shell::ShellExt` y las APIs `tauri_plugin_shell::process::CommandEvent` en su lugar. Lee la guía [Incrustar Binarios Externos](/develop/sidecar/#running-it-from-rust) para ver cómo.
...
Para obtener más información sobre permisos y capacidades, consulta [la documentación](/reference/acl/).