...
1. ##### Start some setup tasks
    ...
    #[tauri::command]
    async fn set_complete(state: State<Mutex<SetupState>>, task: String) -> Result<(), ()> {
    ...
    // And async function that does some heavy setup task
    async fn setup(app: AppHandle) -> Result<(), ()> {
        ...
        set_complete(app.state::<State<Mutex<SetupState>>>(), "backend".to_string()).await;
        Ok(())
    }
    ```
    </ShowSolution>
