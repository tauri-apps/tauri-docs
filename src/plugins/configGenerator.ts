...
export default async function configGenerator() {
	if (fs.existsSync('packages/tauri-v2/tooling/api/node_modules')) {
		build(
			'packages/tauri-v2/core/tauri-config-schema/schema.json',
			'src/content/docs/2/reference/config.md'
		);
	} else {
		throw Error(
	...
	if (fs.existsSync('packages/tauri-v1/tooling/api/node_modules')) {
		build(
			'packages/tauri-v1/core/tauri-config-schema/schema.json',
			'src/content/docs/1/reference/config.md'
		);
	} else {
		throw Error(
			'Tauri V1 submodule is not initialized, respective core configuration routes will not be rendered.'
		);
	}
}

function build(schemaPath: string, targetPath: string) {
...
`;
	...
	function buildObject(key, value, headerLevel: number) {
		let out = [];
